...
async function generateCarouselCopy(story, channelName = '1affairs') {
    if (!process.env.ANTHROPIC_API_KEY) {
        console.log("No ANTHROPIC_API_KEY found, using mock generator...");
        ...
        return {
            ...
            slides: [
                ...
                {
                    text: `According to recent reports, <span class='highlight'>this could change everything.</span>`,
    ...
    The output MUST be valid JSON matching exactly this shape:
    {
        ...
        "slides": [
            ...
            {
                "text": "The next breakdown sentence. Wrap key insight in <span class='highlight'>key insight</span>.",
                "subtext": "Supporting context. | SWIPE"
            },
            {
                "text": "The concluding sentence or takeaway. Wrap impact in <span class='highlight'>impact</span>.",
                "subtext": "What do you think? | READ CAPTION"
            }
        ]
    }
    Generate between 2 and 3 slides. The last slide's subtext must end with '| READ CAPTION', all other slides must end with '| SWIPE'.
